...
export function ListAllTraduction(props) {
  ...
  if (traductions) {
    ...
    if (listingStyle == "block") {
      rendu = traductions.map(traduction => (
        <div
          key={traduction._id}
          className="
						d-flex 
					...
					"
        >
          <BlockStyleTraduction traduction={traduction} />
        </div>
      ));
    } else if (listingStyle == "list") {
      let list = traductions.map(traduction => (
        <li key={traduction._id} className="col-12">
          <ListStyleTraduction traduction={traduction} />
        </li>
      ));
