...
export class ButtonFilterTraduction extends Component {
	...
	render () {
		...
		if (this.props.user || this.props.artist) {
			...
			getMyTraductionFunction = () => this.props.getTraductionByUserId(Meteor.userId());
		}

		return (
			<div className="btn-group pull-right">

				<button className="btn btn-primary" onClick={() => this.props.changeStyle()}>
					{typeDisplay}	
				</button>

				<button className='btn btn-success' onClick={() => getMyTraductionFunction()}>
					{getMyTraductionText}
				</button>
			</div>
		);
